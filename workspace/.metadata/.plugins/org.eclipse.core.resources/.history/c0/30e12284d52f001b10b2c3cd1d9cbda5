...
public class Homework4_Scores {
	...
	public static void main(String[] args) {
		...
		int[][] array= new int[names.length][9];
		
		for(int i=0; i<array.length-1; i++){				//// Random //////////
			for(int j=0; j<array[i].length-2; j++){         // 	array[i][7]는  	//
				array[i][j] = (int)(Math.random() * 101);   //  		 학생별 총점
				array[i][7] += array[i][j];		            // array[names.length-1][j]는 
				array[names.length-1][j] += array[i][j];	            //		과목 합계		//
			}												//////////////////////
		}
		for(int i=0; i<array.length-1;i++){             ////// Ranking ///////////
		...
		System.out.printf("과목합계\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",array[names.length-1][0],array[names.length-1][1],array[names.length-1][2],
				array[names.length-1][3],array[names.length-1][4],array[names.length-1][5],array[names.length-1][6]);
		System.out.printf("과목평균\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\n",
				array[names.length-1][0]/(float)(names.length-1.0),array[names.length-1][1]/(float)(names.length-1.0),array[names.length-1][2]/(float)(names.length-1.0),array[names.length-1][3]/(float)(names.length-1.0),
				array[names.length-1][4]/(float)(names.length-1.0),array[names.length-1][5]/(float)(names.length-1.0),array[names.length-1][6]/(float)(names.length-1.0));
